import React from 'react';
// Similiar to Our History Page
function Discipleship() {
  return (
    <div id="about-us-container">
      <div className="top-img-container">
        <img
          className="top-img-exterior"
          src={require('../imgs/exterior.jpg')}
          alt=""
        />
        <div className="top-img-title">Discipleship</div>
      </div>

      <div id="about-us-header-container">
        <h1>Lorem ipsum dolor sit amet</h1>
        <p>
          Pellentesque habitant morbi tristique senectus et netus et malesuada
          fames ac turpis egestas. Duis pretium facilisis tortor ullamcorper
          lacinia. Nunc elementum ante lacus, quis bibendum massa dignissim
          consequat. Etiam id lectus consectetur, commodo tortor id, laoreet
          ipsum. Fusce vestibulum dictum lacinia. Praesent ut urna vel erat
          ultrices viverra nec sit amet lacus. Pellentesque aliquam dui in
          nisi fringilla dictum.
        </p>
      </div>

      <hr />

      <div className="about-us-item">
        <img 
          className="about-us-imgs" 
          src={require('../imgs/aboutus/1.jpg')}
          alt="" 
        />
        <div className="about-us-text">
          <h2>Donec id odio felis</h2>
          <p>
            Pellentesque habitant morbi tristique senectus et netus et malesuada
            fames ac turpis egestas. Duis pretium facilisis tortor ullamcorper
            lacinia. Nunc elementum ante lacus, quis bibendum massa dignissim
            consequat. Etiam id lectus consectetur, commodo tortor id, laoreet
            ipsum. Fusce vestibulum dictum lacinia. Praesent ut urna vel erat
            ultrices viverra nec sit amet lacus. Pellentesque aliquam dui in
            nisi fringilla dictum.
          </p>
        </div>
      </div>

      <hr />

      <div className="about-us-item">
        <div className="about-us-text">
          <h2>Donec id odio felis</h2>
          <p>
            Pellentesque habitant morbi tristique senectus et netus et malesuada
            fames ac turpis egestas. Duis pretium facilisis tortor ullamcorper
            lacinia. Nunc elementum ante lacus, quis bibendum massa dignissim
            consequat. Etiam id lectus consectetur, commodo tortor id, laoreet
            ipsum. Fusce vestibulum dictum lacinia. Praesent ut urna vel erat
            ultrices viverra nec sit amet lacus. Pellentesque aliquam dui in
            nisi fringilla dictum.
          </p>
        </div>
        <img 
          className="about-us-imgs" 
          src={require('../imgs/aboutus/3.jpg')}
          alt="" 
        />
      </div>

      <hr />

      <div className="about-us-item">
        <img 
          className="about-us-imgs" 
          src={require('../imgs/aboutus/4.jpg')}
          alt="" 
        />
        <div className="about-us-text">
          <h2>Donec id odio felis</h2>
          <p>
            Pellentesque habitant morbi tristique senectus et netus et malesuada
            fames ac turpis egestas. Duis pretium facilisis tortor ullamcorper
            lacinia. Nunc elementum ante lacus, quis bibendum massa dignissim
            consequat. Etiam id lectus consectetur, commodo tortor id, laoreet
            ipsum. Fusce vestibulum dictum lacinia. Praesent ut urna vel erat
            ultrices viverra nec sit amet lacus. Pellentesque aliquam dui in
            nisi fringilla dictum.
          </p>
        </div>
      </div>

      <hr />

      <div className="about-us-item">
        <div className="about-us-text">
          <h2>Donec id odio felis</h2>
          <p>
            Pellentesque habitant morbi tristique senectus et netus et malesuada
            fames ac turpis egestas. Duis pretium facilisis tortor ullamcorper
            lacinia. Nunc elementum ante lacus, quis bibendum massa dignissim
            consequat. Etiam id lectus consectetur, commodo tortor id, laoreet
            ipsum. Fusce vestibulum dictum lacinia. Praesent ut urna vel erat
            ultrices viverra nec sit amet lacus. Pellentesque aliquam dui in
            nisi fringilla dictum.
          </p>
        </div>
        <img 
          className="about-us-imgs" 
          src={require('../imgs/aboutus/5.jpg')}
          alt="" 
        />
      </div>

      <hr />

      <div className="about-us-item">
        <img 
          className="about-us-imgs" 
          src={require('../imgs/aboutus/6.jpg')}
          alt="" 
        />
        <div className="about-us-text">
          <h2>Donec id odio felis</h2>
          <p>
            Pellentesque habitant morbi tristique senectus et netus et malesuada
            fames ac turpis egestas. Duis pretium facilisis tortor ullamcorper
            lacinia. Nunc elementum ante lacus, quis bibendum massa dignissim
            consequat. Etiam id lectus consectetur, commodo tortor id, laoreet
            ipsum. Fusce vestibulum dictum lacinia. Praesent ut urna vel erat
            ultrices viverra nec sit amet lacus. Pellentesque aliquam dui in
            nisi fringilla dictum.
          </p>
        </div>
      </div>

      <hr />

      <div id="about-us-header-container">
        <h1>Lorem ipsum dolor sit amet</h1>
        <p>
          Pellentesque habitant morbi tristique senectus et netus et malesuada
          fames ac turpis egestas. Duis pretium facilisis tortor ullamcorper
          lacinia. Nunc elementum ante lacus, quis bibendum massa dignissim
          consequat. Etiam id lectus consectetur, commodo tortor id, laoreet
          ipsum. Fusce vestibulum dictum lacinia. Praesent ut urna vel erat
          ultrices viverra nec sit amet lacus. Pellentesque aliquam dui in
          nisi fringilla dictum.
        </p>
      </div>

    </div>
  );
}

export default Discipleship;
